...
local MouseLocationLog <const> = StorageManager
...
---@type table<"Pressed"|"Released", boolean>
local ActionConverter <const> = {
    ["Pressed"] = true,
    ["Released"] = false,
};

InputListener:BindAction("TriggerOperatingKeybind", "down", function(name, state, key)
    if state == nil or type(state) ~= "string" then return end
    local StatusBool <const> = ActionConverter[state] or false
    InputListener:SimulateKey("win", StatusBool)
end)

-- print(math.sqrt((0 - 5)^2 + (7 - 10)^2))
...
local function GameTickUpdate(dt)
    ...
    "Delata Time:\t%G\nMOUSE-X:\t%G\nMOUSE-Y:\t%G", dt, InputListener:GetMousePosition())
